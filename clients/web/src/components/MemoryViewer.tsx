      ...
      <div className="px-3 sm:px-8 py-4 sm:py-6 space-y-5">
        ...
        <div className={`p-5 rounded-2xl border ${style.border} bg-zinc-900/30 space-y-3`}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className={`text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-full ${style.badgeBg}`}>
              {style.label}
            </span>
          ...
          {parsed.meta.tags && parsed.meta.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1.5">
              {parsed.meta.tags.map(t => (
                <span key={t} className="flex items-center gap-1 text-[10px] text-zinc-500 bg-zinc-900/60 px-2 py-0.5 rounded-md border border-zinc-800">
                  <Tag size={9} />{t}
                </span>
              ))}
            </div>
