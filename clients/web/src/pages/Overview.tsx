...
import { Activity, AlertCircle, Clock, Cpu, Layers, LayoutDashboard, MessageSquare, Wrench } from 'lucide-react'
import { useWebSocketContext } from '../contexts/WebSocketContext'
import { PageShell, PageHeader, panelCls } from '../components/PageLayout'
...
export default function Overview() {
  ...
  return (
    <PageShell>
      <PageHeader
        title="Overview"
        subtitle="Current client status, recent sessions, and conversation activity."
        icon={<LayoutDashboard size={18} className="text-emerald-400" />}
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className={`${panelCls} p-4 transition-colors ${status === 'connected' ? 'border-emerald-800/40' : status === 'connecting' ? 'border-amber-800/40' : 'border-red-800/40'}`}>
          <div className="flex items-center gap-1.5 text-xs text-zinc-500 mb-1.5">
            <Activity size={12} className={status === 'connected' ? 'text-emerald-400' : status === 'connecting' ? 'text-amber-400' : 'text-red-400'} /> Connection
          </div>
        ...
        <div className={`${panelCls} p-4 transition-colors ${stats.failedTools > 0 ? 'border-red-800/40' : ''}`}>
          <div className="flex items-center gap-1.5 text-xs text-zinc-500 mb-1.5">
            <AlertCircle size={12} className={stats.failedTools > 0 ? 'text-red-400' : ''} /> Failures
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Card title="Capabilities" icon={<Cpu size={15} className="text-zinc-500" />}>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3"><div className="text-xs text-zinc-500">Models</div><div className="text-zinc-200 font-semibold">{models.length}</div></div>
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3"><div className="text-xs text-zinc-500">Tools</div><div className="text-zinc-200 font-semibold">{tools.length}</div></div>
          </div>
        </Card>
