...
export default function Overview() {
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="px-3 sm:px-8 py-5 sm:py-7 space-y-5 page-enter">
        <div>
          <h1 className="text-lg font-semibold tracking-tight text-zinc-100">Overview</h1>
          <p className="text-sm text-zinc-500 mt-1">Current client status, recent sessions, and conversation activity.</p>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className={`rounded-2xl border p-4 shadow-sm transition-colors ${status === 'connected' ? 'border-emerald-800/40 bg-zinc-900/50' : status === 'connecting' ? 'border-amber-800/40 bg-zinc-900/50' : 'border-red-800/40 bg-zinc-900/50'}`}>
          ...
          <div className={`rounded-2xl border p-4 shadow-sm transition-colors ${stats.failedTools > 0 ? 'border-red-800/40 bg-zinc-900/50' : 'border-zinc-800 bg-zinc-900/50'}`}>
            <div className="flex items-center gap-1.5 text-xs text-zinc-500 mb-1.5">
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <Card title="Capabilities" icon={<Cpu size={15} className="text-zinc-500" />}>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-3 shadow-sm"><div className="text-xs text-zinc-500">Models</div><div className="text-zinc-200 font-semibold">{models.length}</div></div>
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-3 shadow-sm"><div className="text-xs text-zinc-500">Tools</div><div className="text-zinc-200 font-semibold">{tools.length}</div></div>
            </div>
          </Card>
