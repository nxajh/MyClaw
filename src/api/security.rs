...
//! `Channel::security_policy()` / `check_authorization`). Phase 3c also
//! moved the evaluation contract (`MessageScope`, `AuthDecision`,
//! `evaluate`) here so the L0 layer is self-contained; only
//! `warn_if_locked_down` stays in `channels` (takes `&dyn Channel`).
...
// ── Authorization contract (moved from channels/security.rs, #151 Phase 3c) ──
