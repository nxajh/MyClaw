...
pub fn build_plan(
    ...
    let root_fqid = migrate_users(&mut plan, data_dir, to_namespace)?;
    migrate_resolver(&mut plan, data_dir, to_namespace, root_fqid.as_deref())?;
...
pub fn run_auto(workspace_dir: &Path, data_dir: &Path, namespace: &str) -> Result<MigrationReport> {
    let plan = build_plan(workspace_dir, data_dir, namespace, true)?;
    if plan.is_empty() {
...
fn migrate_users(plan: &mut MigrationPlan, data_dir: &Path, to_ns: &str) -> Result<Option<String>> {
...
fn migrate_resolver(
    ...
    let path = data_dir.join("user_resolver.json");
    if !path.exists() {
        return Ok(());
    }
    let raw = std::fs::read_to_string(&path)
