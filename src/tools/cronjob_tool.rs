...
impl CronJobTool {
    pub(crate) fn handle_create(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if let Err(e) = scan_prompt_injection(&prompt) {
    ...
    pub(crate) fn handle_update(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(v) = args.get("prompt").and_then(|v| v.as_str()) {
            // Prompt injection scan.
            if let Err(e) = scan_prompt_injection(v) {
                return Ok(err_result(&format!("Prompt rejected: {}", e)));
            }
    ...
    pub(crate) fn handle_list(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
    ...
    pub(crate) fn handle_set_enabled(
        &self,
        args: &serde_json::Value,
        enabled: bool,
    ) -> anyhow::Result<ToolResult> {
    ...
    pub(crate) fn handle_run(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        let id = match args.get("id").and_then(|v| v.as_str()) {
            Some(id) => id.to_string(),
            None => return Ok(err_result("Missing required field: id")),
        };
    ...
    pub(crate) fn handle_remove(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        let id = match args.get("id").and_then(|v| v.as_str()) {
            Some(id) => id.to_string(),
            None => return Ok(err_result("Missing required field: id")),
        };
    ...
    pub(crate) fn handle_log(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        let id = match args.get("id").and_then(|v| v.as_str()) {
            Some(id) => id,
            None => return Ok(err_result("Missing required field: id")),
        };
...
/// six near-duplicates.
pub(crate) fn format_unknown_job_listing(mut jobs: Vec<JobEntry>) -> String {
    if jobs.is_empty() {
...
pub(crate) fn parse_webhook_channel(
    args: &serde_json::Value,
    name: Option<&str>,
...
pub(crate) fn resolve_delivery_for_create(args: &serde_json::Value) -> Result<DeliveryConfig, String> {
    ...
    if let Some(target) = args.get("target").and_then(|v| v.as_str()) {
        return Ok(parse_target_string(target));
    }
...
pub(crate) fn resolve_delivery_for_update(args: &serde_json::Value) -> Result<Option<DeliveryConfig>, String> {
    ...
    if let Some(target) = args.get("target").and_then(|v| v.as_str()) {
        return Ok(Some(parse_target_string(target)));
    }
    Ok(None)
}
