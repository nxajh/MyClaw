...
impl AgentLoop {
    ...
    ///
    /// This is the single choke-point for all generic tool execution:
    /// timeout → execute → truncate. Special tools (ask_user, agent_delegate)
    /// bypass this and are handled directly in `execute_tool`.
    async fn run_tool(
        &self,
        ...
        let raw = if self.config.tool_timeout_secs > 0 {
            ...
            tokio::time::timeout(timeout, tool.execute(args))
                ...
                .unwrap_or_else(|_| Ok(ToolResult {
                    success: false,
                    output: format!("Tool '{}' timed out after {}s", name, self.config.tool_timeout_secs),
...
/// Parse a tool arguments string into a JSON value, tolerating malformed input.
