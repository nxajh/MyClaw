...
// LoopBreakerConfig lives in `crate::api::loop_breaker` (L0 contract, shared
// with config/cli/daemon). Re-exported here so existing
// `agents::loop_breaker::LoopBreakerConfig` paths keep compiling.
pub use crate::api::loop_breaker::LoopBreakerConfig;
...
mod tests {
    ...
    fn max_calls_triggers_hard_limit() {
        ...
        // Alternate tool names to avoid ExactRepeat triggering first.
        for i in 0..5 {
