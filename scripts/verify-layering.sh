...
for mod in ids config str_utils scheduling_types; do
  out=$(grep -rn "crate::\(providers\|memory\|identity\|tools\|agents\|scheduling\|commands\|channels\|daemon\|cli\|websocket\)" src/$mod/ 2>/dev/null || true)
  if [ -n "$out" ]; then
...
for mod in providers memory identity; do
  out=$(grep -rn "crate::\(tools\|agents\|scheduling\|commands\|channels\|daemon\|cli\|websocket\)" src/$mod/ 2>/dev/null || true)
  if [ -n "$out" ]; then
...
# L2 mcp：不引 L4/L5/L6
out=$(grep -rn "crate::\(agents\|scheduling\|commands\|channels\|daemon\|cli\|websocket\)" src/mcp/ 2>/dev/null || true)
if [ -n "$out" ]; then
...
for mod in agents scheduling_runtime commands; do
  out=$(grep -rn "crate::\(channels\|daemon\|cli\|websocket\)" src/$mod/ 2>/dev/null || true)
  if [ -n "$out" ]; then
...
# L5 渠道层：不引 L6
out=$(grep -rn "crate::\(daemon\|cli\|websocket\)" src/channels/ 2>/dev/null || true)
if [ -n "$out" ]; then
