...
| 项 | 方案 |
|---|---|
| 路由 | 同 listener `/api/*`：`GET /api/sessions`、`POST /api/sessions/:id/switch`、`GET /api/memory`…（method 名 → REST 动词的逐条映射表在实现 PR 给出） |
| handler 复用 | `api/sessions.rs`、`api/memory.rs`、`api/skills.rs`、`api/system.rs` 的域函数签名不变，仅换调用方（REST router）与响应包装（`api_response{result}` → HTTP JSON） |
| 鉴权 | 与 WS 同 token：`Authorization: Bearer` |
| 大载荷 | `file.read` base64 改 HTTP 分页（offset/limit），消除 WS 大帧场景 |
| `daemon.restart` | 保留，POST /api/daemon/restart（触发 SIGUSR1，handler 现逻辑不动） |

## 7. 前端改造（clients/web）

- `useWebSocket.ts` 重写为 ACP client：JSON-RPC 帧收发、initialize 握手、session/load、prompt 发起、update 流累积渲染（chunk→文本增量；tool_call/update→工具时间线；thought 折叠区）。
- 管理 API 调用层换 REST fetch。
- 兼容性：此 PR 系列内与 server 同步落地（见 §8 P3），不留中间态。

## 8. 实施批次（每批一 commit 一轮 CI；禁本地 cargo）

| 批次 | 内容 | 破坏性 | 回退 |
|---|---|---|---|
| P1 | REST 管理面：HTTP 路由骨架 + api/ 域函数接线 + 前端 api 层切 REST（WS 旧 api 帧保留） | 无（双轨） | 直接 revert |
| P2 | ACP adapter：`initialize`/`session/*` 编解码 + bus 桥接 + TurnEvent 映射；引入路径路由——`/ws` 走 ACP，其余路径（`/myclaw`）保持旧 type 帧协议（前端零感知） | 无（双轨） | revert；前端未动 |
| P3 | 前端切 `/ws`（ACP）；header/subprotocol 鉴权强制；25MiB 上限；删除旧协议路径与 channel.rs 巨型 match、api/mod.rs WS 分发骨架 | **有**（旧协议下线） | 保留一版热修回退点 |
| P4 | 收尾：tests.rs 旧协议用例迁移为 ACP 一致性测试（crate 类型往返）；文档更新 | 无 | — |
...
| 前端一次性重写量 | P1/P2 期间旧前端持续可用，重写集中在 P3 前置分支 |
| `EmptyResponse` 语义弱化 | 保留告警日志 + 前端空回复提示 |
| REST 与 WS 共用 listener 的 HTTP 解析复杂度 | 升级请求只认 `/ws` 路径 + 标准 header；其余按简单 HTTP/1.1 处理，不引重框架 |
| 开放：`client_id`/设备标识去留 | ACP 无此概念；bus 键已不含设备 id（channel.rs L400-411 注释确认），确认无残留后删除 |

## 10. 验收标准

1. 标准 ACP 客户端（用 agent-client-protocol crate 写的 conformance 脚本）可完成 initialize → session/new → prompt → 收全量 update → response 闭环；
2. 断连重连：prompt 进行中断开，turn 跑完，重连 session/load 后重放缓冲事件；
3. 同身份双连接同时在线，输出广播一致；
4. REST 管理 API 全 25+ method 行为等价（对照现 api 帧响应快照）；
5. 25MiB 超限帧被拒（413/关闭），daemon 内存无尖峰；
6. `myclaw update` 热切换后 WS listener 存活（pre_bound 路径回归）；
7. CI：lint/build/layering/migrate 全绿，测试计数 = 基线 + 新增（ACP 用例）。
